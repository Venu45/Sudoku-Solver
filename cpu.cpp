...
int main(){
	...
	*/
    /*
    vector< vector <  pair< int , pair <int , int> > > >  B;

    // B is 7 X 7

    vector <  pair< int , pair <int , int> > > temp1;
    ...
    for(int ii=0;ii<7;ii++){
        for(int jj=0;jj<7;jj++){
            B[ii].push_back(make_pair(0 , make_pair(ii,jj)));
        }
    } 
    B[0][0].f = 1;
    B[0][3].f = 1;
    ...
    print2(B);
    vector< vector <  pair< int , pair <int , int> > > > empty1;
    vector< vector <  pair< int , pair <int , int> > > >  soln1 = algX(B,7,7,empty1);
   ...
   // cout<<"time duration is : "<<stop-start<<" milliseconds"<<endl;
    //fout.close();
    //g = exactcover;
    return 0;
}